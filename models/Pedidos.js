...
Pedido.obtenerDetalle = async (id_pedido) => {
    const result = await sequelize.query('SELECT productos.nombre, productos.url_foto, productos.precio, pedido_producto.cantidad FROM pedido_producto JOIN productos ON productos.id = pedido_producto.producto_id WHERE pedido_producto.pedido_id = ?',
        {
